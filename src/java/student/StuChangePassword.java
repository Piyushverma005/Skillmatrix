        ...
        try (PrintWriter out = response.getWriter()) {
         ...
         if(db.rst.next()){
            if(npswd.equals(cpswd)){
             ...
             if(i>0){
                 out.println("<h2 style='color:blue;'>Password Update Successfully</h2>");
             }
         }
         else{
             out.println("<h2 style='color:red;'>Error: New and Confirm Password must be same</h2>");
         }
        }
         else{
           out.println("<h2 style='color:red;'>Error:Please Enter Correct Old Password</h2>");
         }
        }
        catch(Exception e){
            e.printStackTrace();
